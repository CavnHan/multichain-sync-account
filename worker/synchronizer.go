...
import (
	...
	"github.com/CavnHan/multichain-sync-account/common/clock"
...
func (syncer *BaseSynchronizer) processBatch(headers []rpcclient.BlockHeader) error {
	...
	for i := range headers {
		...
		if err != nil {
			log.Error("get bock info error", "error", err)
			return err
		}
		businessList, err := syncer.database.Business.QueryBusinessList()
		if err != nil {
			log.Error("query business list fail", "error", err)
			return err
		}
		...
		for _, tx := range txList {
			...
			for _, business := range businessList {
				...
				//deposit
				if !existFromAddress && (existToAddress && toAddressType == 0) {
	...
	headersRef := headers
	syncer.syncerBatches <- &BaseSynchronizerBatch{
